...
class FilterInstanceViewController: BaseFormViewController {
    var index: Int = 0
    ...
    private func appFilters() {
        guard let filters = app.instanceFilters, filters.count > 0 else {
            return
        }
    ...
    private func deleteFilterGroup(_ placementId: String? = nil) {
        ...
        <<< ButtonRow() {
            ...
            var filters = self.app.instanceFilters ?? []
            if filters.count > groupIndex {
                filters.remove(at: groupIndex)
                self.app.instanceFilters = filters
    ...
    @objc func addFiterGroup() {
        deleteFilterGroup(curSlotId)
        groupSection = Section("条件组")
        form +++ groupSection
        var filters = app.instanceFilters ?? []
        filters.append(filterGroup)
        app.instanceFilters = filters
